...
export const setCurrentShow = (events) => {
  ...
  return (dispatch) =>
    fetch("https://public.radio.co/stations/sd8ab6b5aa/status")
      .then((response) => {
        if (!response.ok) {
          throw Error(response.statusText);
        }
      ...
      .then((results) => {
        dispatch({
          type: "SET_CURRENT_SHOW",
          currentShow: {
            title: results.current_track.title || "Jolt Mix",
          },
        });
      })
      .catch(function (error) {
        console.log("Could not fetch title from stream: " + error.message);
      });
};
...
export const getSearch = (query) => {
  return (dispatch) => {
    ...
    fetch(
      JoltSettings.URL.api + "/artists?search=" + query + "&_embed&per_page=12"
    )
      .then(function (response) {
        if (!response.ok) {
          throw Error(response.statusText);
