...
//
add_action( 'init', 'ci_create_cpt_news' );

if( !function_exists('ci_create_cpt_news') ):
function ci_create_cpt_news()
{
	$labels = array(
		'name'               => _x( 'News', 'post type general name', 'ci_theme' ),
		'singular_name'      => _x( 'News', 'post type singular name', 'ci_theme' ),
		'add_new'            => __( 'Add New', 'ci_theme' ),
		'add_new_item'       => __( 'Add New news', 'ci_theme' ),
		'edit_item'          => __( 'Edit news', 'ci_theme' ),
		'new_item'           => __( 'New news', 'ci_theme' ),
		'view_item'          => __( 'View news', 'ci_theme' ),
		'search_items'       => __( 'Search news', 'ci_theme' ),
		'not_found'          => __( 'No news found', 'ci_theme' ),
		'not_found_in_trash' => __( 'No news found in the trash', 'ci_theme' ),
		'parent_item_colon'  => __( 'Parent news:', 'ci_theme' )
	);

	$args = array(
		'labels'          => $labels,
		'singular_label'  => __( 'news', 'ci_theme' ),
		'public'          => true,
		'show_ui'         => true,
		'capability_type' => 'post',
		'hierarchical'    => false,
		'has_archive'     => _x( 'news-archive', 'post type archive slug', 'ci_theme' ),
		'rewrite'         => array( 'slug' => _x( 'news', 'post type slug', 'ci_theme' ) ),
		'menu_position'   => 5,
...
if (false) :
if ( !function_exists( 'ci_cpt_news_meta_box' ) ):
function ci_cpt_news_meta_box( $object, $box ) {
	ci_prepare_metabox( 'cpt_news' );
	...
	?><div class="ci-cf-wrap"><?php
		ci_metabox_open_tab( __('Information', 'ci_theme') );
			...
			<fieldset class="ci-repeating-fields">
				<div class="inner">
					<?php
						$fields = get_post_meta( $object->ID, 'ci_cpt_news_fields', true );

						if ( ! empty( $fields ) ) {
							foreach ( $fields as $field ) {
								?>
								<div class="post-field">
									<label><?php _e( 'Title:', 'ci_theme' ); ?> <input type="text" name="ci_cpt_news_fields_repeatable_title[]" value="<?php echo esc_attr( $field['title'] ); ?>" class="widefat" /></label>
									<label><?php _e( 'Description:', 'ci_theme' ); ?> <input type="text" name="ci_cpt_news_fields_repeatable_description[]" value="<?php echo esc_attr( $field['description'] ); ?>" class="widefat" /></label>
					...
					<div class="post-field field-prototype" style="display: none;">
						<label><?php _e( 'Title:', 'ci_theme' ); ?> <input type="text" name="ci_cpt_news_fields_repeatable_title[]" value="" class="widefat" /></label>
						<label><?php _e( 'Description:', 'ci_theme' ); ?> <input type="text" name="ci_cpt_news_fields_repeatable_description[]" value="" class="widefat" /></label>
						<p class="ci-repeating-remove-action"><a href="#" class="button ci-repeating-remove-field"><i class="dashicons dashicons-dismiss"></i><?php _e( 'Remove me', 'ci_theme' ); ?></a></p>
					</div>
				</div>
